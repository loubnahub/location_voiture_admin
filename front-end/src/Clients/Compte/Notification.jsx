...
import {
  ...
  // Optional: import clear functions if you add buttons for them
...
const ClientNotificationItem = ({ notification, onAction }) => {
  const { id, title, message, timestamp, is_read } = notification;
...
// In NotificationsPage.js, replace both old useEffects with this one:
...
useEffect(() => {
  const loadNotifications = async () => {
    ...
    setError(null);
    try {
      const response = await fetchClientNotifications();
      const allNotifications = response.data || []; // Ensure it's always an array
      ...
      // Now, apply the active filter to the newly fetched data immediately.
  ...
  loadNotifications();
}, []); // This hook only runs ONCE on component mount.

// You still need a separate useEffect to react to FILTER CHANGES.
useEffect(() => {
  // This hook runs ONLY when the user clicks a filter button (activeFilter changes).
  // It uses the 'notifications' state, which is already populated.
  if (activeFilter === 'unread') {
...
}, [activeFilter, notifications]); // Note the dependency array here
