...
// Main container to orchestrate the left/right column animations
const pageVariants = {
  hidden: { opacity: 0 },
  visible: {
    opacity: 1,
...
// Variants for the left column (Contact Info)
...
// Variants for the right column (Form + Map)
...
// Stagger container for list items (like contact details)
...
// Variant for the submission status message
...
export default function ContactPage() {
  // Your state and handlers remain unchanged
  ...
  const handleSubmit = async (e) => {
    ...
    setSubmitMessage("");
    // ... your form submission logic remains exactly the same
  ...
  const contactDetails = {
    location: { title: "Our Location", addressLine1: "Oujda ,Lazaret Street , Morocco", addressLine2: "Recalo, CA 203956" },
    phone: { title: "Phone Number", numbers: ["+212 606604405"], availability: "Mon-Fri from 8am to 8pm" },
    email: { title: "Email Address", address: "contact@Recalo.com", availability: "Always available for support" },
    officeHours: { title: "Office Hours", hours: ["Monday - Friday: 8:00 AM - 8:00 PM", "Saturday: 9:00 AM - 6:00 PM", "Sunday: 10:00 AM - 4:00 PM"] }
  };
          ...
          >
              ...
              <div className="tw-sticky tw-top-24">
                ...
                <motion.div className="tw-space-y-6 tw-mb-10" variants={staggerListContainer}>
                  ...
                  <motion.div className="tw-flex tw-items-start" variants={itemVariants}>
                    ...
                    <div><h3 className="tw-font-medium tw-text-lg tw-mb-1">{contactDetails.location.title}</h3><p className="tw-text-gray-400">{contactDetails.location.addressLine1}<br />{contactDetails.location.addressLine2}</p></div>
                  </motion.div>
                  {/* Phone */}
                  <motion.div className="tw-flex tw-items-start" variants={itemVariants}>
                    ...
                    <div><h3 className="tw-font-medium tw-text-lg tw-mb-1">{contactDetails.phone.title}</h3>{contactDetails.phone.numbers.map((num, idx) => (<p key={idx} className="tw-text-gray-400">{num}</p>))}<p className="tw-text-gray-500 tw-text-sm">{contactDetails.phone.availability}</p></div>
                  </motion.div>
                  {/* Email */}
                  <motion.div className="tw-flex tw-items-start" variants={itemVariants}>
                    ...
                    <div><h3 className="tw-font-medium tw-text-lg tw-mb-1">{contactDetails.email.title}</h3><p className="tw-text-gray-400">{contactDetails.email.address}</p><p className="tw-text-gray-500 tw-text-sm">{contactDetails.email.availability}</p></div>
                  </motion.div>
                  {/* Office Hours */}
                  <motion.div className="tw-flex tw-items-start" variants={itemVariants}>
                    ...
                    <div><h3 className="tw-font-medium tw-text-lg tw-mb-1">{contactDetails.officeHours.title}</h3>{contactDetails.officeHours.hours.map((line, idx) => (<p key={idx} className="tw-text-gray-400">{line}</p>))}</div>
                  </motion.div>
                </motion.div>
                ...
                <motion.div variants={itemVariants}>
                  ...
                  <div className="tw-flex tw-space-x-4">
                    <a href="#" aria-label="Facebook" className="tw-bg-gray-800 tw-p-3 tw-rounded-full hover:tw-bg-gray-700 tw-transition-colors"><Facebook size={18} className="tw-text-blue-400" /></a>
                    <a href="#" aria-label="Twitter" className="tw-bg-gray-800 tw-p-3 tw-rounded-full hover:tw-bg-gray-700 tw-transition-colors"><Twitter size={18} className="tw-text-blue-400" /></a>
                    <a href="#" aria-label="Instagram" className="tw-bg-gray-800 tw-p-3 tw-rounded-full hover:tw-bg-gray-700 tw-transition-colors"><Instagram size={18} className="tw-text-blue-400" /></a>
                  </div>
                </motion.div>
              </div>
            </motion.div>
            ...
            <motion.div className="lg:tw-w-2/3" variants={rightColumnVariants}>
              ...
              <motion.div className="tw-mt-10 tw-rounded-xl tw-overflow-hidden tw-h-72 md:tw-h-96" variants={itemVariants}>
                <iframe src={mapEmbedUrl} width="100%" height="100%" style={{ border: 0 }} allowFullScreen="" loading="lazy" referrerPolicy="no-referrer-when-downgrade" title="Office Location Map"></iframe>
              </motion.div>
            </motion.div>
          </motion.div>
        </div>
